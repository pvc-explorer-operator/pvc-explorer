...
import (
	...
	"github.com/pvc-explorer-operator/pvc-explorer/internal/controller"
	internalscaler "github.com/pvc-explorer-operator/pvc-explorer/internal/scaler"
	uiembed "github.com/pvc-explorer-operator/pvc-explorer/ui"
...
// It defaults to "dev" for local builds without a tag.
var Version = "dev"

func init() {
	utilruntime.Must(clientgoscheme.AddToScheme(scheme))

	utilruntime.Must(pvcexplorerv1alpha1.AddToScheme(scheme))
...
func main() {
	...
	flag.StringVar(&httpAddr, "http-bind-address", ":8080", "The address the API/UI HTTP server binds to.")
	...
	if operatorNS == "" {
		operatorNS = "pvc-explorer-system"
	}
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "9083de2c.pvcexplorer.io",
	})
	if err != nil {
	...
	if err := (&controller.PVCExplorerScopeReconciler{
		...
		setupLog.Error(err, "Failed to create controller", "controller", "pvcexplorerscope")
		os.Exit(1)
	}
	if err := (&controller.PVCExplorerReconciler{
		Client:        mgr.GetClient(),
		Scheme:        mgr.GetScheme(),
		ConsumerIndex: consumer.NewIndexWithBroadcaster(broadcaster),
		Broadcaster:   broadcaster,
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "Failed to create controller", "controller", "pvcexplorer")
		os.Exit(1)
	}
	...
	snapshotFn := func() (any, error) {
		...
		if err := mgr.GetClient().List(context.Background(), &scopes); err != nil {
			return nil, err
		}
		return map[string]any{"explorers": explorers.Items, "scopes": scopes.Items}, nil
	}
	mux.HandleFunc("/ws/v1/events", authMiddleware.WrapFunc(broadcaster.ServeWS(snapshotFn)))
	mux.Handle("/", uiembed.Handler())
	httpServer := &http.Server{
		Addr:    httpAddr,
		Handler: authMiddleware.Wrap(mux),
	}
	go func() {
		setupLog.Info("Starting HTTP server", "addr", httpAddr)
