...
# VERSION is injected into the binary via ldflags. Defaults to git describe output
# so local builds get a version like "v0.1.0-5-gabcdef1-dirty".
...
KIND_CLUSTER ?= pvc-explorer-test-e2e
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name pvc-explorer-builder
	$(CONTAINER_TOOL) buildx use pvc-explorer-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm pvc-explorer-builder
	rm Dockerfile.cross
...
license-check: ## Check Go dependencies for Apache 2.0 license compatibility.
	...
	@echo "✅ All dependencies are compatible with Apache 2.0"

.PHONY: license-report
license-report: ## Generate detailed license report for all dependencies.
	@command -v go-licenses >/dev/null 2>&1 || { echo "go-licenses not installed. Run: go install github.com/google/go-licenses@latest"; exit 1; }
	@mkdir -p dist
	@echo "Generating license report..."
	@go-licenses csv ./... | grep -v "github.com/pvc-explorer-operator/pvc-explorer" > dist/licenses.csv
	@echo "License report generated: dist/licenses.csv"
	@echo ""
	@echo "Summary:"
	@awk -F',' '{print $$2}' dist/licenses.csv | sort | uniq -c | sort -rn
