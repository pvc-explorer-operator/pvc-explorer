# Documentation Index

This directory contains the canonical project documentation for PVC Explorer.

## Getting Started

- [Getting Started](getting-started.md)

## Architecture

- [Architecture](architecture.md)
- [Architecture Decision Records](adr/)

## Development

- [Development](development.md)
- [UI Docs](ui/index.md)

## Security

- [Security Policy and Vulnerability Reporting](../SECURITY.md)

## Operations

- [Branch Protection and Rulesets](operations/branch-protection.md)
- [Codebase Inventory (Current N/A, Future Multi-Repo)](operations/codebase-inventory.md)
- [CRD Installation](operations/crd-installation.md)
- [CRD Release Process](operations/crd-release-process.md)
- [Releases](releases.md)

## Compliance

- [CRA Compliance](compliance/CRA_COMPLIANCE.md)
- [License Compliance](compliance/LICENSE_COMPLIANCE.md)

## Branding

- [Branding Assets](branding/README.md)

## Naming Guidance

- Prefer kebab-case for new markdown files.
- Keep canonical operational docs in `docs/`.
- Use adapter pages in `site/docs/` when site-specific rendering is needed.
